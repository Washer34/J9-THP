...
puts "Dans cet Array il y a #{twitter.count()} handles."
...
shorter = 381
short_index = 0
twitter.count().times do |i|
  length = twitter[i].length
  #puts length
  if length < shorter
    #puts "nouveau handle plus court trouvé : #{twitter[i]}"
    shorter = twitter[i].length
    short_index = twitter[i]
  end
end

puts "le handle le plus court est " + short_index
...
twitter.count().times do |i|
  length = twitter[i].length
  if length == 6
    array_6.push(twitter[i])
  end
end
puts "il y a #{array_6.count} handles contenant 5 caractères :"
p Array(array_6)
...
#twitter.sort_by(&:downcase) <============================================== retirer le commentaire

#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)

#puts twitter.sort_by(&:length) <=========================================== retirer le commentaire

clean
#Quelle est la position dans l'array de la personne @epenser ?

epenser = twitter.find_index {|elem| elem =~ /^@epenser/}
puts "@epenser est à la position : #{epenser}"

#####################################################################################################################################################

def FONCTION_DE_RECHERCHE_DANS_UN_ARRAY #Cherche tout ce qui comporte le word =~ (possibilité de mettre un ancrage avec $)
  twitter.each_with_index do |word, index|
    if word =~ /mot_souhaité/
      puts "#{word} se trouve à #{index}"
    end
  end
end
#####################################################################################################################################################
...
groupes = twitter.group_by {|handle| handle.length}.sort_by {|length, handle| length}

groupes.each do |length, handle|
  puts "#{handle.length} handles avec #{length} caractères"
end