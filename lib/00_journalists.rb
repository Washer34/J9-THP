...
twitter.each do |handle|
  ...
  longueur[handle] = length
end
valeur = 1
longueur.count.times do |valeur|
  if longueur.values.count(valeur) > 1 && valeur != 0
    puts "il y a #{longueur.values.count(valeur)} noms qui ont une length de #{valeur}"
  end
end